...
class MainTab(QWidget):
    ...
    def createBar(self): 
        ...
        self.btn2.clicked.connect(lambda : self.handelFormBtn(MainTab.FORMGROUP))
        self.btn1.clicked.connect(lambda : self.handelFormBtn(MainTab.WELCOMEGROUP))
    ...
    def createFormGroupBox(self):
        ...
        for val in self.options:
            self.combo.addItem(val, userData=QtCore.QVariant(str(self.options[val]['value'])))
        ...
        self.formLayout.addRow(QLabel('Enter The Folder Which Contains The Execl Files.'), self.filebtn)
        self.formLayout.addRow(QLabel("Enter the number of files you want to load from directory"), self.combo)
        self.formLayout.addRow(QLabel('Create Tabs For Execl files.'), self.tabCreatorBtn)
    ...
    def fileBtnHandler(self, x):
        ...
        if self.file_path == None or self.file_path == " " or len(self.file_path) == 0:
            self.file_path = " "
            MainWindow.errorHandler("Directory Not Selected!\n\nplease select directory.")
            return
        ...
        if len(self.exels_files) == 0:
            MainWindow.errorHandler("Directory Contains No .xlsx files.", "Directory Execl file")
            return
...
class MainWindow(QMainWindow):
    ...
    def getInstance():
        if MainWindow._instance != None:
            ...
            MainWindow.errorHandler("Internal Error please Inform Some One!", "Internal Error")
